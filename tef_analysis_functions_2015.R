...
chrDensPlot <- function(chr="1", data=DAG6){
    densChr.10 <- apply(data[data[,1]==chr,2:3], 1, function(cord) seq(as.numeric(cord[1]), as.numeric(cord[2])) )
    print(data[data[,1]==chr,2:3])
    print(length(floor(unlist(densChr.10))))
    hist(floor(unlist(densChr.10)), freq=TRUE, breaks=diff(range(densChr.10)), main=paste("chr.",chr), xlab="10-base occ.")
