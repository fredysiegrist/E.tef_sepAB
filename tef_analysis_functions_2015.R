...
chrDensPlot <- function(chr="1", data=DAG6){
    densChr.10 <- apply(data[data[,1]==chr,2:3], 1,
        function(cord) seq(floor(as.numeric(cord[1])/10), floor(as.numeric(cord[2])/10)))
...
densityMap <- function(data, ink, chromosomes = 1:10, overview=TRUE,
    ...
    if (overview) {
        ...
        xlabl="nt"
        ylabl="scaffolds"
        mainl="E. tef on Sorghum"
        }
    else {
        subprefix="Sorghum chromosome #"
        xlabl="nt position on "
        ylabl="runif distributed E. tef scaffolds"
        mainl="Mapping of E. tef on Sorghum"
        }
    for (chrno in chromosomes) {
        ...
        apply(data[data$chr==chrno, c(2:3,15)], 1, function(z) {
        n<-sample(1:chrlen[chrno],1); x<- z[1:2]; y<-c(n,n); colr <- ink[z[3]]; lines(x, y, col=colr, lwd=2)})
        }
}
...
plotdiff <- function(betterlist, best, slimmaster, connec, penalty) {
    ...
    plot(1:length(connec), (1:length(connec))-iterconnec, pch=16, xlab="ordinal # in better master", ylab="position difference",
    cex=1, main=paste("ordering in master and ", penalty, "master file"), col=colors()[as.numeric(itermaster$chr)+98])
}
